...
namespace Memcury
{
    ...
    public:
        ...
        static auto FindStringRef(T string, bool bWarnIfNotFound = true, int useRefNum = 0, bool bIsInFunc = false, bool bSkunky = false) -> Scanner
        {
            ...
            // Scan the .text section for references
            for (DWORD i = 0x0; i < textSection.GetSectionSize(); i++)
            {
                if ((scanBytes[i] == ASM::CMOVL || scanBytes[i] == ASM::CMOVS) && scanBytes[i + 1] == ASM::LEA)
                {
                    ...
                    if (rdataSection.isInSection(stringAdd))
                    {
                        ...
                        // Check if the first character is printable
                        if (ASM::byteIsAscii(strBytes[0]))
                        {
                            if constexpr (std::is_same<T, std::wstring>::value)
                            {
                                auto lea = stringAdd.GetAs<const wchar_t*>();
                                if (std::wstring(lea) == string) // Compare wide strings
                                {
                                    add = PE::Address(&scanBytes[i]);
                                    if (++aa > useRefNum)
                                        break;
                            ...
                            {
                                auto lea = stringAdd.GetAs<const char*>();
                                if (std::string(lea) == string) // Compare normal strings
                                {
                                    add = PE::Address(&scanBytes[i]);
                                    if (++aa > useRefNum)
                                        break;
                            ...
                            {
                                auto lea = stringAdd.GetAs<const wchar_t*>();
                                if (wcscmp(lea, string) == 0) // Compare raw wide strings
                                {
                                    add = PE::Address(&scanBytes[i]);
                                    if (++aa > useRefNum)
                                        break;
                            ...
                            {
                                auto lea = stringAdd.GetAs<const char*>();
                                if (strcmp(lea, string) == 0) // Compare raw strings
                                {
                                    add = PE::Address(&scanBytes[i]);
                                    if (++aa > useRefNum)
                                        break;
            ...
            // Handle function references if specified
            if (add.Get())
            {
                if (bIsInFunc)
                {
                    ...
                    {
                        if (!bSkunky
                            ? (*(uint8_t*)(add.Get() - i) == 0x48 && *(uint8_t*)(add.Get() - i + 1) == 0x83)
